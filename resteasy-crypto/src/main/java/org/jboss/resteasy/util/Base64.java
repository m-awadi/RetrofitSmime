...
/* ********  E N C O D I N G   M E T H O D S  ******** */
    ...
    private static byte[] encodeBytesToBytes(byte[] source, int off, int len, int options) throws java.io.IOException {
        ...
        if (off < 0) {
            throw new IllegalArgumentException(String.format("Cannot have negative offset: %d", off));
        }   // end if: off < 0
        if (len < 0) {
            throw new IllegalArgumentException(String.format("Cannot have length offset: %d", len));
        }   // end if: len < 0
...
/* ********  D E C O D I N G   M E T H O D S  ******** */
    ...
    private static byte[] decode(byte[] source, int off, int len, int options)
        ...
        if (len == 0) {
            ...
            throw new IllegalArgumentException(String.format("Base64-encoded string must have at least four characters, but length specified was %d", len));
        }   // end if
        byte[] DECODABET = getDecodabet(options);
        ...
        for (i = off; i < off + len; i++) {  // Loop through source
            ...
            if (sbiDecode >= WHITE_SPACE_ENC) {
                ...
                throw new java.io.IOException(String.format("Bad Base64 input character decimal {0} in array position {1}", ((int) source[i]) & 0xFF, i));
            }   // end else:
